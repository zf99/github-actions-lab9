...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
  ...
  echo "Please run 'source delete_product.sh <ID>' to execute the script."
  exit 1
fi
...
echo "Deleting a Product by ID=$ID"
response=$(curl -s -o /dev/null -w "%{http_code}" -X DELETE http://$HOSTNAME:5000/products/$ID)
echo "========"
echo "response"
echo "========"
echo $response
