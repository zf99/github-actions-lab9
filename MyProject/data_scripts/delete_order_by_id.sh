...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
  ...
  echo "Please run 'source delete_order.sh <ID>' to execute the script."
  exit 1
fi
...
echo "Deleting a Order by ID=$ID"
response=$(curl -s -o /dev/null -w "%{http_code}" -X DELETE http://$HOSTNAME:5000/orders/$ID)
echo "========"
echo "response"
echo "========"
echo $response
